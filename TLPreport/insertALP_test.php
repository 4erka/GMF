<?php
 ...
 $kriteria = "";
  ...
  $qryinsert = "INSERT INTO relsummary (Subject, Reference, RefNo, ACType, ATA, Deskripsi, `Status`, InsertProblem, DateIssued, RedAlert) VALUES ";
  $insertvalue = "";
  ...
  for ($i=0;$i<35;$i++)
  {
	...
	if ($actype == "B737-Classic") {
		...
		$qryline = "(`tblpirep_swift`.`REG` Like 'GM%' or `tblpirep_swift`.`REG` Like 'GF%')";
	} else {
	...
	*/
	...
	$sumaction = "";
	foreach ($date as $k => $v) {
		$sumdeskripsi = $sumdeskripsi."\n".$k." On ".$v." PK-".$reg[$k]." experienced ".$problem[$k];
		$sumaction = $sumaction."\n".$k." ".$action[$k];
	}*/

		//$qryinsert = "INSERT INTO relsummary (Subject, Reference, RefNo, ACType, ATA, Deskripsi, `Status`, InsertProblem, DateIssued, RedAlert) VALUES ('$ATAdesc[$i]','ALP','ALP".substr($tahun,2,2).substr($bulan[0],5,2)."','$actype','$ATA[$i]','There ".$tobe." ".$ATApirep[$bulan[0]][$ATA[$i]]." ".$nominal." during ".bulan($bulan[0])." ".$tahun.".',1,'aryan/','".date('Y-m-d')."','$redalert')";
		
		if($insertvalue <> ""){
			$qryinsert .= ",";
		}
		$qryinsert .= "('$ATAdesc[$i]','ALP','ALP".substr($tahun,2,2).substr($bulan[0],5,2)."','$actype','$ATA[$i]','There ".$tobe." ".$ATApirep[$bulan[0]][$ATA[$i]]." ".$nominal." during ".bulan($bulan[0])." ".$tahun.".',1,'aryan/','".date('Y-m-d')."','$redalert')";
		
		$insertvalue = "ts";
		
		//$qryinsert = "INSERT INTO relsummary (Subject, Reference, RefNo, ACType, ATA, Deskripsi, Action, `Status`, InsertProblem, DateIssued, RedAlert) VALUES ('$ATAdesc[i]','ALP','ALP".substr($bulan,-4,4)."','$actype','$ATA[i]','There ".$tobe." ".$ATApirep[$bulan[0]][$ATA[$i]]." ".$nominal." during ".name_bulan($bulan)." ".$tahun.". ".$sumdeskripsi."','$sumaction',1,'aryan/','".date('Y-m-d')."','$redalert')";
