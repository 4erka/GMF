...
function fleet($time,$actype)
{
	if ($actype == "B737-Classic") {
		...
		$qryline = "(`tbl_monthlyfhfc`.`Reg` Like 'PK-GM%' or `tbl_monthlyfhfc`.`Reg` Like 'PK-GF%')";
...
function alert($timeawal,$timeakhir,$actype)//query untuk Alert Level Pilot Report
{
	...
	if ($actype == "B737-Classic") {
		...
		$qryline1 = "(`tbl_monthlyfhfc`.`Reg` Like 'PK-GM%' or `tbl_monthlyfhfc`.`Reg` Like 'PK-GF%')";
	} else {
...
function ifsdlastyear($timeawal,$timeakhir,$actype)
{
	...
	$qryFH = "SELECT `tbl_ifsd`.`ACType`, DATE_FORMAT(`tbl_ifsd`.`DateOccur`,'%Y-%m') AS `periode`, COUNT(`tbl_ifsd`.`ID`) AS `jumlah` FROM `tbl_ifsd` WHERE  ".$qryline." AND (DATE_FORMAT(`tbl_ifsd`.`DateOccur`,'%Y-%m') BETWEEN '".$timeawal."' AND '".$timeakhir."') AND `tbl_ifsd`.`ifsd` = 'IFSD' GROUP BY DATE_FORMAT(`tbl_ifsd`.`DateOccur`,'%Y-%m')";
	$resultFH = mysql_query($qryFH);
	//die($qryFH);
	//while ($hasilFH = mysql_fetch_array($resultFH))
	//{
...
function rto($time,$actype)
{
	...
	$qryFH = "SELECT `tbl_ifsd`.`ACType`, DATE_FORMAT(`tbl_ifsd`.`DateOccur`,'%Y-%m') AS `periode`, COUNT(`tbl_ifsd`.`ID`) AS `jumlah` FROM `tbl_ifsd` WHERE  ".$qryline." AND DATE_FORMAT(`tbl_ifsd`.`DateOccur`,'%Y-%m') <= '".$time."' AND `tbl_ifsd`.`rto` = 'RTO' GROUP BY DATE_FORMAT(`tbl_ifsd`.`DateOccur`,'%Y-%m')";
	$resultFH = mysql_query($qryFH);
	//die($qryFH);
	while ($hasilFH = mysql_fetch_array($resultFH))
	{
...
function rtolastyear($timeawal,$timeakhir,$actype)
{
	...
	$qryFH = "SELECT `tbl_ifsd`.`ACType`, DATE_FORMAT(`tbl_ifsd`.`DateOccur`,'%Y-%m') AS `periode`, COUNT(`tbl_ifsd`.`ID`) AS `jumlah` FROM `tbl_ifsd` WHERE  ".$qryline." AND (DATE_FORMAT(`tbl_ifsd`.`DateOccur`,'%Y-%m') BETWEEN '".$timeawal."' AND '".$timeakhir."') AND `tbl_ifsd`.`rto` = 'RTO' GROUP BY DATE_FORMAT(`tbl_ifsd`.`DateOccur`,'%Y-%m')";
	$resultFH = mysql_query($qryFH);
	while ($hasilFH = mysql_fetch_array($resultFH))
	{
		$hour[$hasilFH["periode"]] = $hasilFH["jumlah"];
...
function ATApirepperformance($time,$actype)//query untuk  Report
{
	if ($actype == "B737-Classic") {
		...
		$qryline = "(`tblpirep`.`REG` Like 'PK-GM%' or `tblpirep`.`REG` Like 'PK-GF%')";
	} else {
		$qryline = "`tblpirep`.`ACTYPE` IN ('$actype')";
	}
