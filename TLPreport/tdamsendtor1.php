...
foreach ($get as $a => $b){
	...
	{	
		mysql_query("INSERT INTO tblflweng (Eventid, nopeg, Status, unit) VALUES ('".substr($k,-8)."', '".$newget[substr($k,-8)][2]."', 'Open', '".$newget[substr($k,-8)][0]."')");
		
		$resultinsert = mysql_query("SELECT TDAMNo FROM tdamx WHERE ID = '".substr($k,-8)."'");
		while ($hsl = mysql_fetch_array($resultinsert)){
			$tdamnno = $hsl["TDAMNo"];
		}
		mysql_query("INSERT INTO tbltdameng (Eventid, tdamno, Nopeg, unit, Date) VALUES ('".substr($k,-8)."', '".$tdamnno."', '".$newget[substr($k,-8)][1]."', '".$newget[substr($k,-8)][0]."', '".date('Y-m-d')."')");
	}
}
...
{
	$server = '192.168.240.105';
	$username = 'emptemp';
	$password = 'emptemp';
	$password2 = 'PASSWORD';
	$database = 'employee_back';
